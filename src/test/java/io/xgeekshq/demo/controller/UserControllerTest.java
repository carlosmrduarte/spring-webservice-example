...
class UserControllerTest {
    ...
    void createUser(@Autowired ObjectMapper jsonMapper) throws Exception {
        ...
        this.mvc
                ...
                .andExpect(
                        jsonPath("$.[*].gender")
                                .value(Matchers.hasItem(dto.getGender().getValue())))
                .andExpect(
                        jsonPath("$.[*].birthDate")
                                .value(Matchers.hasItem(dto.getBirthDate().toString())));
    }

    @Test
    void getUserById() throws Exception {
        this.mvc
                .perform(get("/api/user/{userId}", 1).accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isNotEmpty())
                ...
                .andExpect(
                        jsonPath("$.birthDate")
                                .value(LocalDate.of(1992, Month.APRIL, 10).toString()));
    }
